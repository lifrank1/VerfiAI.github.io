        ...
        <div className="references-container">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h4 className="references-title">
              References {paper.references && paper.references.length > 0 ? `(${paper.references.length})` : ''}
            </h4>
          </div>
...
const CitationStats = ({ citations }) => {
  ...
  useEffect(() => {
    const verifyAllCitations = async () => {
      ...
      const stats = results.reduce((acc, status) => {
        ...
        else if (status === 'not_found' || status === 'failed') acc.unverifiable++;
        else acc.notVerified++;
        return acc;
      }, { verified: 0, notVerified: 0, unverifiable: 0 });
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Citation Verification Status',
        color: '#333',
        font: {
          size: 16
        }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <p><strong>Total Citations:</strong> {citations.length}</p>
        <p><strong>Verified:</strong> {verificationStats.verified} ({Math.round(verificationStats.verified/citations.length * 100)}%)</p>
        <p><strong>Not Verified:</strong> {verificationStats.notVerified} ({Math.round(verificationStats.notVerified/citations.length * 100)}%)</p>
        <p><strong>Unverifiable:</strong> {verificationStats.unverifiable} ({Math.round(verificationStats.unverifiable/citations.length * 100)}%)</p>
      </div>
    </div>
  );
};

// First, add this new component for the verification chart in messages
const VerificationPieChart = ({ references }) => {
  const [verificationStats, setVerificationStats] = useState({
    verified: 0,
    notVerified: 0,
    unverifiable: 0,
      ...
      {verificationStats.loading ? (
        ...
        <>
          ...
          <div style={{
            ...
            <p><strong>Total References:</strong> {references.length}</p>
            <p><strong>Verified:</strong> {verificationStats.verified} ({Math.round(verificationStats.verified/references.length * 100)}%)</p>
            <p><strong>Not Verified:</strong> {verificationStats.notVerified} ({Math.round(verificationStats.notVerified/references.length * 100)}%)</p>
            <p><strong>Unverifiable:</strong> {verificationStats.unverifiable} ({Math.round(verificationStats.unverifiable/references.length * 100)}%)</p>
          </div>
        </>
      )}
      <button
        onClick={verifyAllReferences}
        style={{
          background: '#6E44FF',
          color: 'white',
          border: 'none',
